...
export const verificationStatusCheck = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = await User.findOne({ where: {email: req.body.email}});
    if (!user) throw new Error("User not authorized");
    if (!user.verification_status) throw new Error("User has been deactivated, Contact admin");
    next();
  } catch (error) {
    if (error instanceof Error) {
      next(createHttpError(403, error))
